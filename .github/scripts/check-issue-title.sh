...
if ! [[ "$title" =~ $pattern ]]; then
  ...
  echo "::error::  feat(storefront): add public active services endpoint" >&2
  echo "::error::  fix(auth): reject inactive tenant login" >&2
  echo "::error::  ci(github): validate pr title references" >&2
  echo "::error::Got: $title" >&2
  exit 1
fi

if [[ "$title" == *"+"* ]]; then
  echo "::error::Issue title uses '+' shorthand." >&2
  echo "::error::Write 'and' between two items or commas between three or more." >&2
  echo "::error::Got: $title" >&2
  exit 1
fi
